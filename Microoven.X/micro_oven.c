...
void display_power_screen(void)
{
    ...
    {
        clcd_putch(BAR,LINE1(i)); //0 1 2 3
        __delay_ms(100);
    ...
    clcd_print("Microwave Oven", LINE3(2));
     for(unsigned char i = 0;i < 16 ; i++)
    {
        clcd_putch(BAR,LINE4(i)); //0 1 2 3
        __delay_ms(100);
...
void set_time(unsigned char key, unsigned char reset_flag)
{
    ...
    else if(key == '#')
    {
        clear_screen();
        FAN = ON ;//turn ON Fan 
        TMR2ON = 1;//turn ON time
        screen_flag = DISPLAY_TIME;//change the screen to display time
    }
...
void set_temp(unsigned char key,unsigned char reset_flag)
{
    ...
    if(display_blank)//to blink temparature
    {
       clcd_putch(' ',LINE2(7));
       clcd_putch(' ',LINE2(8));
       clcd_putch(' ',LINE2(9));  
